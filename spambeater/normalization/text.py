...
def delete_another_trash_symbols(text:str) -> str:
    ...
    eng = r"A-Za-z"
    extra = r"\d\s"
    pattern = re.compile(fr"[^{rus}{eng}{kaz}{extra}]+")
    return pattern.sub(' ',text).strip()
...
russian_letters = 'ёЁ'+''.join([generate_letters_range(*r) \
                           for r in [('а','я'),('А','Я'),('0','9')]])
english_letters = ''.join([generate_letters_range(*r) \
...
greets = [
    'всем','привет','добрый','день','здравствуйте',
    'здравствуй','приветствую','доброго','времени','суток',
]
trash_words = [
    'http','https','это','нe',
]
