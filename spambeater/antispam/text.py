...
class TextPredictModel(PredictModelBase):
    ...
    def predict_spam(self, text:str) -> bool: 
        try:
            ...
            vector = np.array([self._vectorize_text(text),])
    ...
    def _vectorize_text(self,sentence:str):
        if not(self.vectorize_model): return self.zero_vector
        words = sentence.split()
        words_vecs = [self.vectorize_model.wv[w] \
            for w in words if w in self.vectorize_model.wv]
        if len(words_vecs) == 0: return self.zero_vector 
    ...
    def _save_vectorizer(self) -> None:
        if not(self.vectorize_model): return
        try:
    ...
    def get_normalized_df_by_category(self, df: pd.DataFrame, k:int=1) -> pd.DataFrame:
        ds = df.copy()
