...
async def save_message_data(
    ...
    # save text and media
    if text:
        await SaveManager.save_text(text=text, category=category)
    if not (media_file):
        return
...
def get_media_file(message: Message) -> MediaFile | None:
    photo = message.photo
    video = message.video
    document = message.document or message.animation or message.sticker
    if photo:
        media = MediaFile(
...
async def processing_photo(file_bytes: bytes) -> Spam:
    ...
    job = tasks.predict_images_spam.delay(
        images=[
            image,
        ]
    )
...
async def processing_video(file_bytes: bytes, spam_proba: float = 0.2) -> Spam:
    ...
    images = [
        normalize_image(i) for i in await get_main_frames_from_video(video=file_bytes)
    ]
    if not (images):
        return Spam(False)
...
async def antispam_handler(_: Client, message: Message) -> None:
    ...
    if is_command(message=message):
        return
    if not (is_chat_for_processing(chat_id)):
        return
    ...
    media_file = get_media_file(message=message)
    if text:
        spam = await processing_text(text=text)
    if spam:
        await processing_spam_message(message=message, spam_class=spam.cls)
    if not (media_file) or spam:
        return
    file_bytes = await get_file_bytes(file_id=media_file.fid)
    if not (file_bytes):
        return
